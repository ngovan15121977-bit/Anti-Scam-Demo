...
def _persist_assessment(
    ...
    # Phase 2: optional Bank Risk Manager overlay (RISK_MANAGER_ENABLED)
    from src.app.services.risk_manager.integration import (
        maybe_apply_manager_to_transaction,
    )

    signal_types = [s.signal_type for s in candidates]
    score, level, explanation = maybe_apply_manager_to_transaction(
        score=score,
        level=level,
        explanation=explanation,
        signal_types=signal_types,
        requires_hitl=should_warn,
        user_id_hash=str(current_user.id),
        active_guardian_action=(
            active_guardian.agent_action if active_guardian is not None else None
        ),
        active_guardian_score=(
