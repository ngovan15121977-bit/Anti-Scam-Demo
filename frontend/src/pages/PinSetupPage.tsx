  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-600 to-violet-800 flex items-center justify-center p-4">
      <form onSubmit={save} className="w-full max-w-md rounded-3xl bg-white p-8 shadow-2xl">
        <div className="mx-auto mb-5 flex h-16 w-16 items-center justify-center rounded-full bg-indigo-100"><ShieldCheck className="h-8 w-8 text-indigo-600" /></div>
        ...
        <button disabled={saving || !/^\d{4,6}$/.test(pin) || pin !== confirm} className="mt-6 w-full rounded-xl bg-indigo-600 py-3 font-bold text-white disabled:opacity-50">{saving ? "Đang lưu..." : "Tạo PIN và tiếp tục"}</button>
      </form>
    </div>
  );
}
